...
class PerformanceChartGenerator:
    ...
    def create_combined_accuracy_chart(self, save_path=None):
        ...
        x = np.arange(len(accuracy_df))
        width = 0.25  # Reduced width of the bars to prevent overlap
        
        # Create bars for both accuracy types
        bars1 = ax.bar(x - width/2, accuracy_df['Type Accuracy (%)'], 
                      width, label='Type Accuracy', color='#2E8B57', alpha=0.8)
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(accuracy_df['Type Full Name'], rotation=45, ha='right', fontsize=14)
        ax.legend(fontsize=14, loc='upper right', bbox_to_anchor=(0.98, 0.98))
        ax.grid(True, alpha=0.3)
        ax.set_ylim(0, 120)  # 增加Y轴上限，为图例留出更多空间
        ...
        for bars in [bars1, bars2]:
            for bar in bars:
                height = bar.get_height()
                # Position labels inside the bars if they're tall enough, otherwise above
                if height > 20:
                    ax.annotate(f'{height:.1f}%',
