...
class PerformanceChartGenerator:
    ...
    def create_type_accuracy_chart(self, save_path=None):
        """
        Create type accuracy visualization chart.
        ...
        # Create bar chart for type accuracy only
        x = np.arange(len(accuracy_df))
        
        bars = ax.bar(x, accuracy_df['Type Accuracy (%)'], 
                     color='#2E8B57', alpha=0.8, width=0.6)  # 增加柱子宽度，让柱子更协调
        
        # Customize the chart
        ax.set_xlabel('Verification Types', fontsize=16, fontweight='bold')
        ax.set_ylabel('Type Accuracy (%)', fontsize=16, fontweight='bold')
        ax.set_title('Type Recognition Accuracy by IoT Configuration Type', 
                    fontsize=16, fontweight='bold', pad=20)
        ax.set_xticks(x)
        ax.set_xticklabels(accuracy_df['Type Full Name'], rotation=45, ha='right')
        ax.grid(True, alpha=0.3)
        ax.set_ylim(0, 110)  # 增加Y轴上限，为数字标签留出更多空间
        ...
        if save_path:
            ...
            print(f"Type accuracy chart saved to: {save_path}")
        
        plt.show()
        return fig
    
    def create_verification_accuracy_chart(self, save_path=None):
        """
        Create verification accuracy visualization chart.
        ...
        # Create bar chart for verification accuracy only
        x = np.arange(len(accuracy_df))
        
        bars = ax.bar(x, accuracy_df['Verification Accuracy (%)'], 
                     color='#4169E1', alpha=0.8, width=0.6)  # 调整柱子宽度，使其更协调
        
        # Customize the chart
        ax.set_xlabel('Verification Types', fontsize=16, fontweight='bold')
        ax.set_ylabel('Verification Accuracy (%)', fontsize=16, fontweight='bold')
        ax.set_title('Verification Accuracy by IoT Configuration Type', 
                    fontsize=16, fontweight='bold', pad=20)
        ax.set_xticks(x)
        ax.set_xticklabels(accuracy_df['Type Full Name'], rotation=45, ha='right')
        ax.grid(True, alpha=0.3)
        ax.set_ylim(0, 110)  # 调整Y轴范围，为数字标签留出空间
        ...
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"Verification accuracy chart saved to: {save_path}")
    ...
    def create_time_chart(self, save_path=None):
        ...
        bars2 = ax.bar(x, time_df['Config Time (s)'], bottom=bottom, label='Config Time',
                      color=colors[1], alpha=0.8, width=bar_width)
    ...
    def generate_all_charts(self, output_dir="Graphs"):
        ...
        # Generate charts
        print("Generating type accuracy chart...")
        type_accuracy_path = Path(output_dir) / "type_accuracy.png"
        self.create_type_accuracy_chart(save_path=str(type_accuracy_path))
        
        print("Generating verification accuracy chart...")
        verification_accuracy_path = Path(output_dir) / "verification_accuracy.png"
        self.create_verification_accuracy_chart(save_path=str(verification_accuracy_path))
